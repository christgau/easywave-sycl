...
static inline std::vector<cl::sycl::event> memcpy(cl::sycl::queue &q, void *to_ptr, const void *from_ptr,
    cl::sycl::range<3> to_range, cl::sycl::range<3> from_range, cl::sycl::id<3> to_id, cl::sycl::id<3> from_id,
    cl::sycl::range<3> size)
{
	std::vector<cl::sycl::event> event_list;

	size_t to_slice = to_range.get(1) * to_range.get(0), from_slice = from_range.get(1) * from_range.get(0);
	unsigned char *to_surface = (unsigned char *)to_ptr + to_id.get(2) * to_slice + to_id.get(1) * to_range.get(0) + to_id.get(0);
	const unsigned char *from_surface = (const unsigned char *)from_ptr + from_id.get(2) * from_slice + from_id.get(1) * from_range.get(0) + from_id.get(0);
...
static inline std::vector<cl::sycl::event> memcpy(cl::sycl::queue &q, void *to_ptr, size_t to_pitch,
        const void *from_ptr, size_t from_pitch, size_t x, size_t y)
{
	return memcpy(q, to_ptr, from_ptr,
            cl::sycl::range<3>(to_pitch, y, 1), cl::sycl::range<3>(from_pitch, y, 1),
...
template <typename T> bool in_vec(const T x, const std::vector<T> v) {
    ...
    kernel_duration({0})
{
	...
	cl::sycl::property_list prop_list = is_profiling_enabled
	...
	if (num_sub_devs > 0 && use_subdevices) {
		...
		if (zib::in_vec(sycl::info::partition_property::partition_equally, part_props)) {
			...
			subdevs = root_device.create_sub_devices<sycl::info::partition_property::partition_by_affinity_domain>(
	...
	std::cout << "SubDev[0]'s maximum work group size: "
		  << queues.front().get_device().get_info<cl::sycl::info::device::max_work_group_size>() << std::endl;
...
CGpuNode::~CGpuNode()
{
	if (is_profiling_enabled) {
		...
		for (int i = 0; i < kernel_duration.size(); i++) {
			std::cout << "runtime kernel " << i << " (" << kernel_names[i] << "): " << std::fixed
				  << std::setprecision(3) << kernel_duration[i] << " ms (" << std::fixed << std::setprecision(3)
				  << (kernel_duration[i] / total) << ")" << std::endl;
		}
...
int CGpuNode::run()
{
	...
	kernel_events[KERNEL_WAVE_BOUND].push_back(
	    queues[0].parallel_for(cl::sycl::nd_range<1>(boundary_size, boundary_workgroup_size),
		kernel_events[KERNEL_WAVE_UPDATE], [=](cl::sycl::nd_item<1> item) {
	...
	kernel_events[KERNEL_FLUX_BOUND].push_back(
	    queues[0].parallel_for(cl::sycl::nd_range<1>(boundary_size, boundary_workgroup_size),
		kernel_events[KERNEL_FLUX_UPDATE], [=](cl::sycl::nd_item<1> item) {
	...
	kernel_events[KERNEL_EXTEND].push_back(
	    queues[0].parallel_for(cl::sycl::nd_range<1>(boundary_size, boundary_workgroup_size), extend_deps,
		[=](cl::sycl::nd_item<1> item) {
			gridExtend(kernel_data, item);
	}));

	cl::sycl::int4 MinMax;
	kernel_events[KERNEL_MEMCPY].push_back(
	    root_queue.memcpy(&MinMax, data.g_MinMax, sizeof(cl::sycl::int4), kernel_events[KERNEL_EXTEND]));
	...
	for (int i = 0; is_profiling_enabled && i < NUM_TIMED_KERNELS; i++) {
		...
		for (const auto &e : kernel_events[i]) {
			durations.push_back((e.get_profiling_info<cl::sycl::info::event_profiling::command_end>() -
