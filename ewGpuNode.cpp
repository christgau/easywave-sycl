...
int CGpuNode::mallocMem() {
  ...
  dpct::dpct_malloc(&(data.tArr), &pitch, nJ_aligned * sizeof(float), dp.nI);
	...
	/* 1-dim */
  (data.cR6) = (float *)sycl::malloc_device(dp.nJ * sizeof(float),
                                            dpct::get_current_device(),
                                            dpct::get_default_context());
  (data.cB1) = (float *)sycl::malloc_device(dp.nI * sizeof(float),
                                            dpct::get_current_device(),
                                            dpct::get_default_context());
  (data.cB2) = (float *)sycl::malloc_device(dp.nJ * sizeof(float),
                                            dpct::get_current_device(),
                                            dpct::get_default_context());
  (data.cB3) = (float *)sycl::malloc_device(dp.nI * sizeof(float),
                                            dpct::get_current_device(),
                                            dpct::get_default_context());
  (data.cB4) = (float *)sycl::malloc_device(dp.nJ * sizeof(float),
                                            dpct::get_current_device(),
                                            dpct::get_default_context());

  (data.g_MinMax) = (sycl::int4 *)sycl::malloc_device(
      sizeof(sycl::int4), dpct::get_current_device(),
      dpct::get_default_context());
...
int CGpuNode::copyToGPU() {
	...
	/* 2-dim */
  dpct::dpct_memcpy(data.d + dp.lpad, pitch, d, dp.nJ * sizeof(float),
                    dp.nJ * sizeof(float), dp.nI, dpct::host_to_device);
  dpct::dpct_memcpy(data.h + dp.lpad, pitch, h, dp.nJ * sizeof(float),
                    dp.nJ * sizeof(float), dp.nI, dpct::host_to_device);
  dpct::dpct_memcpy(data.hMax + dp.lpad, pitch, hMax, dp.nJ * sizeof(float),
                    dp.nJ * sizeof(float), dp.nI, dpct::host_to_device);
  dpct::dpct_memcpy(data.fM + dp.lpad, pitch, fM, dp.nJ * sizeof(float),
                    dp.nJ * sizeof(float), dp.nI, dpct::host_to_device);
  dpct::dpct_memcpy(data.fN + dp.lpad, pitch, fN, dp.nJ * sizeof(float),
                    dp.nJ * sizeof(float), dp.nI, dpct::host_to_device);
  dpct::dpct_memcpy(data.cR1 + dp.lpad, pitch, cR1, dp.nJ * sizeof(float),
                    dp.nJ * sizeof(float), dp.nI, dpct::host_to_device);
  dpct::dpct_memcpy(data.cR2 + dp.lpad, pitch, cR2, dp.nJ * sizeof(float),
                    dp.nJ * sizeof(float), dp.nI, dpct::host_to_device);
  dpct::dpct_memcpy(data.cR4 + dp.lpad, pitch, cR4, dp.nJ * sizeof(float),
                    dp.nJ * sizeof(float), dp.nI, dpct::host_to_device);
  dpct::dpct_memcpy(data.tArr + dp.lpad, pitch, tArr, dp.nJ * sizeof(float),
                    dp.nJ * sizeof(float), dp.nI, dpct::host_to_device);
...
int CGpuNode::copyFromGPU() {
  ...
  dpct::dpct_memcpy(hMax, dp.nJ * sizeof(float), data.hMax + dp.lpad, pitch,
                    dp.nJ * sizeof(float), dp.nI, dpct::device_to_host);
  dpct::dpct_memcpy(tArr, dp.nJ * sizeof(float), data.tArr + dp.lpad, pitch,
                    dp.nJ * sizeof(float), dp.nI, dpct::device_to_host);
...
int CGpuNode::copyIntermediate() {
  ...
  dpct::dpct_memcpy(h, dp.nJ * sizeof(float), data.h + dp.lpad, pitch,
                    dp.nJ * sizeof(float), dp.nI, dpct::device_to_host);
...
int CGpuNode::copyPOIs() {
    ...
    dpct::get_default_queue()
        .memcpy(h + idxPOI[n], data.h + dp.lpad + id, sizeof(float))
        .wait();
        }
...
int CGpuNode::run() {
  ...
  */
  ...
  sycl::int4 MinMax;
  dpct::get_default_queue()
      .memcpy(&MinMax, data.g_MinMax, sizeof(sycl::int4))
      .wait();
  dpct::get_current_device().queues_wait_and_throw();

  if (static_cast<int>(MinMax.x()))
    Imin = dp.iMin = std::max(dp.iMin - 1, 2);

  if (static_cast<int>(MinMax.y()))
    Imax = dp.iMax = std::min(dp.iMax + 1, dp.nI - 1);

  if (static_cast<int>(MinMax.z()))
    Jmin = dp.jMin = std::max(dp.jMin - 32, 2);

  if (static_cast<int>(MinMax.w()))
